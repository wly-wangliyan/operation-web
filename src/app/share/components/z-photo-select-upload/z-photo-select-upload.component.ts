...
@Component({
  selector: 'app-z-photo-select-upload',
  templateUrl: './z-photo-select-upload.component.html',
  styleUrls: ['./z-photo-select-upload.component.css']
})
export class ZPhotoSelectUploadComponent implements OnInit {
  ...
  @Input() public imageAddWidth = null; // 添加图片的宽

  @Input() public imageAddHeight = null; // 添加图片的高
  ...
  @Output() public uploadFinish = new EventEmitter(); // 上传完成
  ...
  public ngOnInit() {
    ...
    this.imageAddWidth = this.imageAddWidth ? this.imageAddWidth : this.imageHeight;
  }
  ...
  // 选择查看上一张/下一张图片
  public selectedShowImage(flag: any): void {
    if (flag) {
  ...
  // 关闭图片查看模态框
  public closeShowZoomPicture(): void {
    timer(0).subscribe(() => {
          ...
          if (imageList[index].sourceFile) {
            ...
            if (file.size > (this.limitImgSize * 1024 * 1024)) {
              ...
              this.selectedImgChange.emit({
                ...
                errMsg: '图片大小不得高于' + this.limitImgSize + 'M！'
              });
  ...
  private checkUpload() {
    timer(1000).subscribe(() => {
      this.upload().subscribe(() => {
        const imageUrls = this.imageList.map(i => i.sourceUrl);
        this.uploadFinish.emit({ isUpload: true, file_id: this.file_id, imageList: imageUrls.join(','), errMsg: '' });
      }, err => {
  ...
  private upLoadErrMsg(err: any): void {
    if (!this.globalService.httpErrorProcess(err)) {
      if (err.status === 422) {
        this.uploadFinish.emit({ isUpload: false, file_id: this.file_id, errMsg: '参数错误，可能文件格式错误！' });
      } else if (err.status === 413) {
        this.uploadFinish.emit({ isUpload: false, file_id: this.file_id, errMsg: '上传资源文件太大，服务器无法保存！' });
      } else {
        this.uploadFinish.emit({ isUpload: false, file_id: this.file_id, errMsg: '上传失败，请重试！' });
      }
    }
  }
}
