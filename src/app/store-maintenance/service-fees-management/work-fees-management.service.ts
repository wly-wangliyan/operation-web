...
export class WorkFeesParams extends EntityBase {
  public project_id: string = undefined; // string 项目ID
  public project_name: string = undefined; // string 项目名
...
export class WorkFeesManagementService {
  ...
  /**
   * 获取服务费列表
   * @param SearchWorkFeesParams 条件检索参数
   * @returns Observable<ParkingLinkResponse>
   */
  public requestServiceFeeListData(SearchWorkFeesParams: SearchWorkFeesParams): Observable<ServiceFeeLinkResponse> {
    const httpUrl = `${this.domain}/service_fees`;
    return this.httpService.get(httpUrl, SearchWorkFeesParams.json())
   ...
   * @param params WorkFeesParams 参数
   * @param fee_type number 救援类型
   * @returns Observable<HttpResponse<any>>
   */
  public requestUpdateFeeData(params: WorkFeesParams, service_fee_id: string, fee_type: number): Observable<HttpResponse<any>> {
    const httpUrl = `${this.domain}/service_fees/${service_fee_id}`;
