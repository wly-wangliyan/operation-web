...
export class ParkingEntity extends EntityBase {
  ...
  public toEditJson(): any {
    ...
    if (json.spot_types.includes('1')) {
      json.indoor_origin_fee = json.indoor_origin_fee ? Math.round(json.indoor_origin_fee * 100) : json.indoor_origin_fee;
      json.indoor_sale_fee = json.indoor_sale_fee ? Math.round(json.indoor_sale_fee * 100) : json.indoor_sale_fee;
      json.indoor_pre_fee = json.indoor_pre_fee ? Math.round(json.indoor_pre_fee * 100) : json.indoor_pre_fee;
      json.indoor_minus_fee = json.indoor_minus_fee ? Math.round(json.indoor_minus_fee * 100) : json.indoor_minus_fee;
    } else {
    ...
    if (json.spot_types.includes('2')) {
      json.origin_fee = json.origin_fee ? Math.round(json.origin_fee * 100) : json.origin_fee;
      json.sale_fee = json.sale_fee ? Math.round(json.sale_fee * 100) : json.sale_fee;
      json.pre_fee = json.pre_fee ? Math.round(json.pre_fee * 100) : json.pre_fee;
      json.minus_fee = json.minus_fee ? Math.round(json.minus_fee * 100) : json.minus_fee;
    } else {
