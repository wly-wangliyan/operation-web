...
import { ServiceConfigService, ParkingEntity, AddParkingParams } from '../../service-config.service';
...
export class AddCarParkingComponent implements OnInit {
    ...
    public onSaveParkingIds() {
        ...
        if (checkedParkingList.length === 0) {
            ...
            const addParkingParams = new AddParkingParams();
            addParkingParams.beian_parking_ids = checkedParkingList.map(i => i.parking_id).join(',');

            this.serviceConfigService.requestAddParkingIds(addParkingParams).subscribe(() => {
