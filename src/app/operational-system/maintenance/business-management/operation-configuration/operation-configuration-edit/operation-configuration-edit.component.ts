...
export class OperationConfigurationEditComponent implements OnInit {
    ...
    this.businessManagementService.requestUpkeepProjectList(this.upkeep_merchant_id, this.upkeep_merchant_product_id)
      .subscribe(res => {
        ...
        if (this.upkeep_merchant_product_id_copy) {
          this.requestCopyUpkeepProjectList();
        }
      }, err => {
  ...
  private requestCopyUpkeepProjectList() {
      ...
      res.forEach((result: any) => {
        ...
        result.forEach(value => {
          ...
          const param = {
            ...
            upkeep_merchant_project_id_copy: value.upkeep_merchant_project.upkeep_merchant_project_id,
            upkeep_merchant_id: this.upkeep_merchant_id,
            upkeep_product_id: this.upkeep_merchant_product_id,
          };
          this.projectItemList.forEach(value1 => {
            if (value1.upkeep_merchant_project_id_copy === value.upkeep_merchant_project.upkeep_merchant_project_id) {
              param.upkeep_merchant_project_id_copy = value1.source.upkeep_merchant_project_id;
              this.copyParams.push(param);
            }
          });
        });
      });
  ...
  // 关联配件列表
  public requestRelateAccessories() {
    // 拷贝数组
    const httpList = [];
    this.copyParams.forEach(value => {
      httpList.push(this.businessManagementService.requestCopyUpkeepAccessories(value));
    });
  ...
  public onEditClick(data: UpkeepMerchantAccessoryEntity) {
    ...
    this.businessManagementService.requestUpdateUpkeepAccessories(this.upkeep_merchant_id,
      this.upkeep_merchant_product_id, data.upkeep_merchant_project.upkeep_merchant_project_id, data.upkeep_merchant_accessory_id, params)
      .subscribe(() => {
  ...
  public onDelClick(data: UpkeepMerchantAccessoryEntity) {
    this.businessManagementService.requestDeleteUpkeepAccessory(this.upkeep_merchant_id,
      this.upkeep_merchant_product_id, this.currentProjectId, data.upkeep_merchant_accessory_id)
      .subscribe(() => {
        this.globalService.promptBox.open('删除成功！', () => {
          this.accessoryItemList = this.accessoryItemList.filter(
            value => value.source.upkeep_merchant_accessory_id !== data.upkeep_merchant_accessory_id);
          const index = this.projectItemList.findIndex(
            v => v.source.upkeep_merchant_project_id === data.upkeep_merchant_project.upkeep_merchant_project_id);
  ...
  // 选择配件
  public onChooseClick(event, project_id?: string) {
    const params = {
