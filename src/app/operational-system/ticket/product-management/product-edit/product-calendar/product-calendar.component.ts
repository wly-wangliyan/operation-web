...
export class ProductCalendarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.searchText$.pipe(
      ...
      this.priceCalendarList = res.results.map(i => ({
        ...i,
        platform_price: (Number(i.platform_price) / 100).toFixed(2)
      }));
      const calendarList = this.priceCalendarList.map((value) => [new Date(value.date).getDate(), value]);
      const map = new Map();
  ...
  // 获得某月的天数
  private getMonthDays(myMonth: any) {
