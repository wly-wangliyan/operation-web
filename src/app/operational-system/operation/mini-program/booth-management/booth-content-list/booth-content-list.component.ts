                    ...
                    if (err.status === 422) {
                        ...
                        for (const content of error.errors) {
                            if (content.field === 'status' && content.code === 'missing_field') {
                                ...
                                if (boothContent.image_type === 1) {
                                    this.globalService.promptBox.open(`只能同时开启2个内容!`, null, 2000, null, false);
                                } else if (boothContent.image_type === 2) {
                                    this.globalService.promptBox.open(`只能同时开启4个内容!`, null, 2000, null, false);
                                } else { // 其他展位类型
