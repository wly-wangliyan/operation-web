...
@Component({
    selector: 'app-parking-space-select-tag',
    templateUrl: './parking-space-select-tag.component.html',
    styleUrls: ['./parking-space-select-tag.component.css']
})
export class ParkingSpaceSelectTagComponent {
    ...
    public onSelectTagData() {
        ...
        this.tagList.forEach(item => {
            if (this.selectTagList.indexOf(item.parking_place_label_id) > -1) {
    ...
    public onChangeTag(tag: TagManagementEntity) {
        const findIndex = this.selectTagList.findIndex(item => item === tag.parking_place_label_id);
        if (findIndex > -1) {
            this.selectTagList.splice(findIndex, 1);
        } else {
            this.selectTagList.push(tag.parking_place_label_id);
        }
    }
